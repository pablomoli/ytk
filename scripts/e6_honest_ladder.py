...
— and reruns the E5 sweep: alpha over {0,1,3,7,15,31}, the frozen snapshot on
disk, fixed decay*batch base, within-source shuffle null (1000, seed 0) for
the honest arm. Gate: the ADJUSTED arm at alpha=7 must reproduce the stored
theme weights, because the snapshot is generated by post-E4 production
(medium_controlled on) until this section's ladder merges — section 27 gated
on raw for the same reason, against a pre-regeneration snapshot. The honest
arm is what production computes after the merge; raw is derived by stripping
the playlist lift.
...
def gather() -> dict:
    ...
    max_diff = float(np.max(np.abs(np.round(shares["adjusted"][_akey(REF)], 4) - stored)))
    assert max_diff == 0.0, f"adjusted alpha=7 does not reproduce the snapshot (diff {max_diff})"
    validation = {
...
def fig01(out: dict) -> None:
    ...
    # json round-trips Counter keys to strings; normalize so --figs-only agrees
    hc = {str(kk): vv for kk, vv in out["validation"]["honest_signal_counts"].items()}
    fig, top = figure(
        14,
        8.2,
        1,
        ...
        f"{{0: {hc.get('0', 0)}, 1: {hc.get('1', 0)}, 2: {hc.get('2', 0)}}}  ·  "
    ...
    ax.text(
        0.03,
        0.05,
        "band: the honest arm's within-source shuffle null p5-p95, this freeze\n"
        "(raw and corrected arms carried their own nulls in section 27's freeze)",
        color=MUTED,
        fontsize=7.5,
    ...
    verdict(
        fig,
        "record the intent and the knob stops mattering: tau(0,31) 0.882, inside its own "
        "shuffle band",
    )
...
def fig02(out: dict) -> None:
    ...
    verdict(
        fig,
        "the honest head keeps the corrected top-3 and stops needing the knob: 4 of 5 slots "
