  ...
  useEffect(() => {
    ...
    const place = (w: number, h: number) => {
      ...
      corners.forEach((corner, i) =>
        gsap.to(corner, { x: positions[i].x, y: positions[i].y, duration: 0.18, ease: "house" }),
      );
    };
    ...
    const move = (event: MouseEvent) => {
      ...
      const overText = (event.target as HTMLElement).closest(
        "input, textarea, [contenteditable], pre, .note-panel",
      );
