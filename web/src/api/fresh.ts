...
export function useDeleteNote() {
  return useMutation({
    mutationFn: (path: string) => apiSend<{ deleted: boolean }>('/api/note/delete', 'POST', { path }),
    onSuccess: (_, path) => {
      queryClient.setQueryData<FreshNote[]>(['fresh'], (notes) => notes?.filter((note) => note.path !== path))
    },
  })
}
