...
export type SettingsConfig = {
  ...
  hub: {
    ...
    imessage_gap_minutes: number;
    tags: string[];
    pinterest_feeds: string[];
    enrich_tone: string;
  };
...
export type SettingsResponse = {
  ...
  meta: {
    ...
    last_pulls: Record<string, number>;
    last_pull_at?: number;
...
export const saveSettings = (config: SettingsConfig) =>
  apiSend<{ saved: boolean; restart_required: boolean }>("/api/settings", "PUT", config);
