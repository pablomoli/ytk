...
export function useProfile() {
  return useQuery({ queryKey: ['profile'], queryFn: () => apiGet<Profile>('/api/profile'), retry: false })
}
...
export function useGroveBuckets() {
  return useQuery({ queryKey: ['grove-buckets'], queryFn: () => apiGet<GroveBuckets>('/api/grove-buckets') })
}

export function useSaveGroveBuckets() {
  return useMutation({
    mutationFn: (text: string) =>
      apiSend<{ saved: boolean; buckets: string[]; hint: string }>('/api/grove-buckets', 'PUT', { text }),
