...
function MapPage() {
  ...
  return (
    <div className="map-page">
      <HubControls><button className={`fchip${view === 'all' ? ' on' : ''}`} onClick={() => resetView('all')}>everything</button><button className={`fchip${view === 'content' ? ' on' : ''}`} onClick={() => resetView('content')}>content</button><button className={`fchip${signal ? ' on' : ''}`} onClick={() => setSignal((current) => !current)}>signal</button><button className={`fchip${recent ? ' on' : ''}`} onClick={() => setRecent((current) => !current)}>recent</button><button className="fchip" onClick={() => setFlat((current) => !current)}>{flat ? '3d' : '2d'}</button>{map.data?.all.terrain || map.data?.content.terrain ? <button className={`fchip${terrain ? ' on' : ''}`} onClick={() => { setWeb(false); setTerrain((current) => !current) }}>terrain</button> : null}{map.data?.all.web || map.data?.content.web ? <button className={`fchip${web ? ' on' : ''}`} onClick={() => { if (!web) { setTerrain(false); setFlat(false) } setWeb((current) => !current) }}>web</button> : null}{map.data?.all.fog || map.data?.content.fog ? <button className={`fchip${fog ? ' on' : ''}`} onClick={() => { if (!fog) { setTerrain(false); setFlat(false) } setFog((current) => !current) }}>fog</button> : null}{fog ? <button className={`fchip${fogShell ? ' on' : ''}`} onClick={() => setFogShell((current) => !current)} title="show only the shell |density - level| < 0.06: a pseudo-isosurface at the slider's level">shell</button> : null}{fog ? <input className="fog-level" type="range" min={0} max={0.9} step={0.01} value={fogLevel} onChange={(event) => setFogLevel(Number(event.target.value))} title={fogShell ? 'isosurface level: slide to sweep the shell through the density field' : 'density threshold: slide up to shrink the fog to its cores'} /> : null}<span className="count">{map.data?.points.length ?? 0} notes</span></HubControls>
