...
export function generateDataTree(params: GroveParams, rand: () => number, origin: Vector3, topo: BucketTopology, maxNodes: number): TreeNode {
  ...
  const root: TreeNode = { position: origin.clone(), weight: 1, pathLength: 0, dir: up.clone(), children: [] }
  ...
  const growLimb = (from: TreeNode, dir: Vector3, weight: number, steps: number): TreeNode => {
    ...
    for (let i = 0; i < steps && budget > 0; i++) {
      const pull = direction.clone().add(randomUnit(rand).multiplyScalar(params.noise * 0.8)).add(up.clone().multiplyScalar(params.upBias * 0.25)).normalize()
      direction = direction.clone().multiplyScalar(params.stiffness).add(pull.multiplyScalar(1 - params.stiffness)).normalize()
      ...
      w *= params.girthDecay
      const child: TreeNode = { position, weight: w, pathLength: node.pathLength + step.length(), dir: direction.clone(), children: [] }
  ...
  const stepsFor = (n: TopoNode) => Math.max(3, Math.round(3 + 9 * (n.persistence / maxPersistence)))
  ...
  const walk = (topoNode: TopoNode, fromTree: TreeNode, baseDir: Vector3) => {
    ...
    kids.forEach((kid, i) => {
      ...
      const lateral = side.clone().multiplyScalar(Math.cos(azimuth)).add(out.clone().multiplyScalar(Math.sin(azimuth)))
      const dir = baseDir.clone().multiplyScalar(1 - polar * 0.8).add(lateral.multiplyScalar(polar)).add(up.clone().multiplyScalar(params.upBias * 0.3)).normalize()
