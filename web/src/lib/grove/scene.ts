...
import { leafFragment, leafVertex, lineVertex, tubeFragment, tubeVertex, xrayLineFragment, xrayTubeFragment } from './shaders'
...
export function mountGrove(canvas: HTMLCanvasElement, params: GroveParams, look: GroveLook): GroveHandle {
  ...
  const uniforms = {
    ...
    uPaletteTravel: { value: params.paletteTravel }, uPaletteMotion: { value: params.paletteMotion }, uPaletteStrength: { value: params.paletteStrength },
    uWireGlow: { value: params.wireGlow }, uWirePulse: { value: params.wirePulse }, uWireBody: { value: params.wireBody },
  }
  ...
  const paletteUniforms = (topic: string, requested: string | undefined, root: boolean) => {
    ...
    return {
      uPaletteA: { value: vector(p.a) }, uPaletteB: { value: vector(p.b) }, uPaletteC: { value: vector(p.c) }, uPaletteD: { value: vector(p.d) },
      uPaletteOffset: { value: paletteOffset(topic) }, uPalettePhase: { value: palettePhase(topic) }, uRoot: { value: root ? 1 : 0 },
    }
  }
  const remember = (material: ShaderMaterial) => { treeMaterials.push(material); return material }
  const tubeMaterialsFor = (topic: string, requested: string | undefined, root = false) => {
    ...
    return {
      foliage: remember(new ShaderMaterial({ vertexShader: tubeVertex, fragmentShader: tubeFragment, uniforms: { ...uniforms, ...topicUniforms }, side: DoubleSide })),
      xray: remember(new ShaderMaterial({ vertexShader: tubeVertex, fragmentShader: xrayTubeFragment, uniforms: { ...uniforms, ...topicUniforms }, side: DoubleSide, transparent: true, blending: AdditiveBlending, depthWrite: false })),
    }
  }
  const leafMaterialFor = (topic: string, requested?: string) => remember(new ShaderMaterial({ vertexShader: leafVertex, fragmentShader: leafFragment, uniforms: { ...uniforms, ...paletteUniforms(topic, requested, false) }, side: DoubleSide }))
  const lineMaterialFor = (topic: string, requested: string | undefined, root = false) => remember(new ShaderMaterial({ vertexShader: lineVertex, fragmentShader: xrayLineFragment, uniforms: { ...uniforms, ...paletteUniforms(topic, requested, root) }, transparent: true, blending: AdditiveBlending, depthWrite: false }))
  ...
  const tagged = <T extends Mesh | LineSegments>(object: T, role: 'wood' | 'root' | 'line' | 'root-line' | 'leaves', foliage?: ShaderMaterial, xray?: ShaderMaterial) => {
  ...
  const plant = (next: GroveParams) => {
    ...
    for (let t = 0; t < treeCount; t++) {
      ...
      const treeParams = bucket ? { ...next, reach: lastParams.reach * sizeScale, girth: lastParams.girth * (0.5 + 0.5 * sizeScale) } : next
      ...
      const tree = buildTreeGeometry(treeParams, bucket
      ...
      grown.push(tagged(new Mesh(tubeGeo(tree), woodMaterials.foliage), 'wood', woodMaterials.foliage, woodMaterials.xray))
      grown.push(tagged(new LineSegments(lineGeo(tree), lineMaterialFor(topic, requestedPalette)), 'line'))
      // root system: the same organism grown the opposite way - shorter
      // reach, inverted up bias, a touch gnarlier, darker wood
      const rootParams = { ...treeParams, reach: treeParams.reach * 0.8, upBias: -0.45, initialChildren: Math.max(2, treeParams.initialChildren), branchChance: Math.min(0.6, treeParams.branchChance + 0.1), noise: treeParams.noise * 1.25, stepScale: treeParams.stepScale * 0.8, girth: treeParams.girth * 1.1, stiffness: treeParams.stiffness * 0.85 }
      ...
      still(rootMaterials.foliage); still(rootMaterials.xray)
      grown.push(tagged(new Mesh(tubeGeo(roots), rootMaterials.foliage), 'root', rootMaterials.foliage, rootMaterials.xray))
      grown.push(tagged(new LineSegments(lineGeo(roots), still(lineMaterialFor(topic, requestedPalette, true))), 'root-line'))
      ...
      // looks only changes visibility/materials and never regenerates topology.
      {
        ...
        const leaves = new InstancedMesh(leafGeometry, leafMaterialFor(topic, requestedPalette), count)
  ...
  const applyLook = () => {
    grown.forEach((object) => {
      ...
      object.visible = role === 'leaves' ? currentLook === 'foliage' : role === 'line' || role === 'root-line' ? currentLook === 'x-ray' : true
      if (role === 'wood' || role === 'root') {
        object.material = currentLook === 'x-ray' ? object.userData.xrayMaterial : object.userData.foliageMaterial
      }
    })
  }
  ...
  return {
    ...
    setEffects: (next) => { lastParams = next; applyEffects(next) },
