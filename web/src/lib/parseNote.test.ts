...
test('parses a full youtube note: frontmatter, lead, and sections in order with correct kinds', () => {
  ...
  expect(parsed.frontmatter.title).toBe('Loads of New Blender Experiments! (Working on My Short Film)')
  ...
  expect(headings).toEqual(['Thesis', 'Commentary', 'Key Concepts', 'Insights', 'Key Moments', 'Transcript'])
