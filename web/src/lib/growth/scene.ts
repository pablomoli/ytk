...
const opsA = (dna: SeedDNA) =>
  new Vector3(dna.operators.DEEPEN, dna.operators.BUD, dna.operators.LACE);
const opsB = (dna: SeedDNA) =>
  new Vector3(dna.operators.STIPPLE, dna.operators.BLEED, dna.operators.MEMBRANE);
...
export function mountWorkbench(
  ...
  const render = (now: number) => {
    ...
    if (!paused && spec) {
      ...
      for (const slot of slots) {
        ...
        const steps =
          slot === stage ? dnaToRD(slot.dna).steps : Math.min(6, dnaToRD(slot.dna).steps);
  ...
  return {
    setOrganism(next) {
      ...
      const settled = next.events.slice(0, replayed).map((e, i) => eventFor(e.kind, i, e.title));
