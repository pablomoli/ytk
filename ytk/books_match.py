    ...
    for i, ch in enumerate(raw):
        ...
        for c in d:
            if c.isspace():
                if prev_space:
                    continue
                out.append(' '); n2r.append(i); prev_space = True
            else:
                out.append(c); n2r.append(i); prev_space = False
    while out and out[-1] == ' ':
        out.pop(); n2r.pop()
    return ''.join(out), n2r
...
STOPWORDS = set("""a an the and or but if then else when of to in on at by for with
without from into onto upon as is are was were be been being am do does did have
has had not no nor so than that this these those it its it's he she they them his
her their our your my me you i we us who whom which what whose how why where here
there all any some such very too more most much many few little own same other
will would shall should can could may might must i'm you're we're""".split())
