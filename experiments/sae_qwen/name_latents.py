"""Name specific latents by id and merge them into features.json.

Rung 0 of #183 needs the protagonist note's loudest latents named; they sit
outside the top-100 frequency head that features.py covered.

YTK_VISUAL_INDEX=off uv run python experiments/sae_qwen/name_latents.py \
    --ckpt checkpoints/final_d2048_k32_s0.pt --latents 1597,1211,1310
"""
...
def main() -> None:
    ...
    ap.add_argument("--latents", required=True, help="comma-separated latent ids")
    a = ap.parse_args()

    from ytk.sdk import structured

    want = {int(x) for x in a.latents.split(",")}
    ...
    blob.setdefault("named_by_request", []).extend(sorted(want))
