...
views of figure 01 up as surfaces: bilinearly interpolated between the grid
values (the 32×32 nodes are exact, the surface lerps between them),
hillshaded, row 0 facing the camera. The corpus mean becomes a landform: a
smooth ramp falling away from its 32 loudest dims. One note is weather on
that landform — noise riding a tilted floor, brightest peaks along the row-0
edge — and the gold ridge on the back wall (figure 01's row profile, now a
literal silhouette) starts above the DIM null rails and decays into them.
Center the note and the ramp is gone: the silhouette never leaves the rails,
but the weather is untouched. Same claim as figure 01, now visible as
geography rather than read off a chart.
