...
Before improving how a system remembers things, it is worth measuring what it has already stored. This section is that measurement, done first and deliberately cheaply: every figure here comes from data the production pipeline was already producing, so nothing had to be built or generated to answer the question. It asks how many stored memories are near-duplicates of each other, whether their timestamps mean anything, and what the real rhythm of capture looks like once test traffic is excluded.

Baseline figures on data quality in `ytk_memories` — the memory collection of `ytk`, my personal knowledge system, which ingests YouTube, Instagram and web content into an Obsidian vault, indexes it for semantic search, and stores short captured notes-to-self alongside it. Duplicate density, timestamp integrity, and capture-pattern analysis. Rung 0 refers to the first and cheapest step of a laddered plan filed as issue #150 in this repository's tracker; `R`, `E` and `A` prefixes below mark refinement, experiment and audit items within it.
