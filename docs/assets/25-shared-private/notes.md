...
Question (2026-08-08): sections 22–24 established that the two lenses measure
different things — Qwen embeddings group notes by TOPIC (source purity 0.72),
the gemma-2-2b + gemma-scope SAE fingerprints group them by VOICE/REGISTER
(0.95, baseline 0.61), cross-space ARI 0.239 against a measured within-space
ceiling of 0.335. E3 asks the formal version of "can we derive something from
BOTH": decompose the paired views into SHARED structure, QWEN-PRIVATE (topic
net of voice) and SAE-PRIVATE (voice net of topic). Reference: SPLICE
(arXiv 2408.12091) — linear CCA first, by design.

Generated by `scripts/e3_shared_private.py`. Every number in
`shared-private.json`.
...
```
scripts/e3_shared_private.py                     the whole pipeline
docs/assets/25-shared-private/
  01-shared-spectrum.png                         spectrum, null, PCA sweep, subspace size
  02-medium-or-topic.png                         the E2 prediction test + named loadings
  03-payoff.png                                  private re-clustering battery
  shared-private.json                            every number above
  feature-names.json                             227 Neuronpedia auto-names (cache; delete to refetch)
```
