...
fifty-three numbered sections, each one folder with one `README.md`, holding the
...
| 53 | [syntax-laws](assets/53-syntax-laws/README.md) | the forest, rung 0: before drawing the repo's code as trees, do its syntax trees obey the branching laws real trees and rivers obey? | two of three pre-registered laws hold and the null sharpens both: subtree sizes are Zipfian at slope −1 in every file (216 files, 301k named nodes) while degree-matched random trees give −1.5, so −1 is the grammar's, not the tree's; real trees are bushes not spines, depth ∝ n^0.15 against the null's n^0.47 with 215/216 files below their own null; and Horton's constancy — which the null obeys almost exactly, R_L ≈ 1.9 flat — breaks in one direction, bifurcation rising with order (3.4 → 5.7) and stream length falling below 1 by order 3; the pilot's "two branching regimes" was an artifact of counting nodes instead of streams and is withdrawn, its direction surviving as the length ratio; three drawing rules derived (radius from size, length from the falling ratio, height cannot carry size) |
