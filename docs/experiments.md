...
fifty-eight numbered sections, each one folder with one `README.md`, holding the
...
| 41 | [vector-search](assets/41-vector-search/README.md) | the index we depend on is a black box we never built once — brute-force 10M real-geometry vectors on a 16GB MacBook, then earn the speed back stepwise, recall priced at every step | the cliff is the disk (98s a query, 20 batched cost one sweep); int8 keeps 98% of the truth for 3x; and the weekend-2 scoreboard is two hits, three misses, one split — centering and random rotation are provable no-ops on PQ (the cone is a direction, not a coordinate), PQ128 doubles recall to 0.337 but codes stay candidate generators, IVF's curve never reaches 0.90-inside-5% while the cone taxes its lists at 10x isotropic skew (Gini 0.091 vs 0.009), hnswlib at 1M is fast and *wrong* (0.602 at ef 200 — the graph index is not solved on this geometry either), and the Rust kernel passes both bars: 5.8s int8 sweep (5.5x numpy), 0.32s ADC gather (7.5x) |
...
| 44 | [atlas-binning](assets/44-atlas-binning/README.md) | #183 rung 3: bin the frozen map into cells with latent identities — and how much of each identity survives retraining, is head-explained, or was never seen? | 62 cells carry 52 distinct identities (the map has local vocabulary; #977 EpicMap labels a five-cell continent while the content side fragments); the gate passes 36/62 at decoder cosine 0.5 (20 at 0.8) and the instability is geography, not noise — stable cells sit where head-explained mass is high (up to 50% work-side vs ~10% content-side), rung 2's frequency-buys-stability one level up; median head mass 25% makes the dark-matter caveat a per-cell number; the protagonist postdates the frozen layout so its cell is a disclosed 10-NN estimate — and #1597's excess terrain has exactly one ridge (1/62 cells outside null) with that estimated home sitting on it, two independent instruments agreeing unasked |
| 45 | [segment-strips](assets/45-segment-strips/README.md) | #183 rung 4: the flagged wall — do segment codes aggregate back to their document, and does timeline order carry signal? | no wall, and the premise was off: the SAE trained mostly on segments (11,412 of 16,483 vectors), so the feared distribution shift was unlikely by construction — but the gates ran anyway and both pass: segment-mean codes point at their own document (median cosine 0.67 vs 0.02 mismatched, 18/314 inside null reach) and order is signal (lag-1 autocorr 0.28 real vs -0.04 shuffled, 307/314 videos above their own null); the protagonist's strip splits register from material — its document code led with #1597's educational-breakdown while its segments speak #1206 neural-network-mechanics, #114 neuron-activation-interpretability, #729 residual-connections: the document sees the lecture, the segments see the material |
| 46 | [protagonist-trace](assets/46-protagonist-trace/README.md) | #183 rung 5: the missing genre — one real note traced through every representation the system holds | the AlexNet note end to end: 1024 numbers drawn 32x32, 32 lit pixels of 2,048 (60% of code mass named, 32% head — dark matter stated, not footnoted), an atlas cell independently labeled mechanistic-interpretability, top-5 neighbors that the topic lens and register lens share 3/5 (corpus median 3/10) — Blum's mini-ChatGPT livecode, "Let's build GPT", "The Scaling Hypothesis" — and a great-circle journey into an EpicMap note where #977 rises from exact zero at t=0.33 and overtakes #1597 near the midpoint: interpolation is a feature handover, not a fade, watchable on one pair of real notes |
| 47 | [gen-gate](assets/47-gen-gate/README.md) | #183 rung 7: can a linear Qwen-to-CLIP translator earn the right to render latents as images? | no — the fourth registered loss: mean top-10 neighbor agreement 0.22 on 1,015 held-out notes against the bar of 0.40 that was committed before the translator existed; the signal is real (23x the shuffled-pairs control, with a tail of notes above 0.7) but a renderer right about a minority cannot stamp its images trustworthy, so GEN mode stays closed, no unCLIP weights were ever downloaded, and the atlas keeps its two honest modes — TEXT is measurement, IMG is evidence, and there is no paraphrase |
| 48 | [semantic-compass](assets/48-semantic-compass/README.md) | rung 7 replacement, part 1: can zero-parameter contrast axes give every object — note, latent, cell, corpus — one signature glyph? | a registered loss with a surviving instrument: 3 of 5 axes pass their held-out gates (scroll-sit 0.90, mine-world 0.97, fresh-settled 0.92, shuffle nulls ~0.50) against a bar of 4 — spoken-written scored 0.95 but died of a 21-note pole, code-prose was degenerate at 10 positives; signature stability passed (0.92 across disjoint defining halves) and the surviving near-orthogonal compass reads true on real objects (the AlexNet note reads SIT+WORLD, EpicMap's direction reads MINE, the corpus mean collapses to a dot); adoption and re-registration are owner calls, and no bar moved after the data |
| 49 | [latent-portraits](assets/49-latent-portraits/README.md) | rung 7 replacement, part 2: if a latent's portrait is composited purely from its exemplars' real thumbnails, is it an identity or mush? | the fifth registered loss, and the first caught by eye: the gate scored 0.97 and the owner asked why the same image kept appearing in the hub — segments inherit their parent video's thumbnail, 40% of a typical latent's top-24 exemplars were one image repeated, and "disjoint halves" were scoring a thumbnail's agreement with itself; deduplicated to one exemplar per note the AUC collapses 0.97 -> 0.43 and same-latent halves (r 0.16) are no more alike than strangers (0.20), so composited portraits are dead as an instrument; the same one-per-note rule now governs section 43's mosaics (45.9% distinct-image coverage, not the inflated 55.9%) and the hub's feature cards, and the passport keeps only an honest face — the most central real exemplar, evidence displayed, nothing derived |
| 50 | [constellations](assets/50-constellations/README.md) | owner's conjecture: canonicalize the code image's dimension and the 32 lit latents become a shape — is the shape real? | yes, at 89% against a 60% bar: with every latent frozen at its PCA-of-decoder position, a note's active latents huddle (activation-weighted decoder coherence, median 0.029) far beyond 100 frequency-matched null draws per note (median p95 0.014); drawn as edges past decoder cosine 0.15 the property becomes connectivity — a work note webs at 45 edges, the protagonist holds 25, a listicle frays to 12, chance is dust at 4; the section also ships the owner's share transform — softmax over ranked winners at T = the measured background-pair std (0.106), raw cosine always printed beside it, the temperature carried as data so it is never re-typed |
| 51 | [clip-identity](assets/51-clip-identity/README.md) | 49R (#192): was section 49's portrait claim wrong, or just its pixel judge? re-run the identical gate with the perceptually aligned judge | both — FAIL at AUC 0.60 vs the fresh 0.80 bar (pixel judge scored 0.43): same-latent halves cosine 0.894, cross pairs 0.887, because in CLIP space every YouTube thumbnail resembles every other — the genre is its own cone; disclosed post-hoc diagnostics show centering lifts only to 0.64 and the rest is structural, 505 unique images serving 2,045 latents with 11% image overlap between random pairs — a ~500-item visual vocabulary cannot individuate a 2,048-concept dictionary under any judge, so the portrait metaphor closes for good and the passport's face stays a real exemplar |
| 52 | [compass-regrow](assets/52-compass-regrow/README.md) | 48R (#193): section 48 lost the compass 3-2 on two thin poles — did those axes die of the corpus, or of their recipes? | of their recipes, and one of them died of its name: re-pulled from the live store the old loss replicates exactly (growth moved the written pole 21 -> 28 and the code pole 10 -> 10), and Chroma turns out to embed a YouTube note's enrichment rather than its transcript, so every doc vector is composed prose and section 48's spoken-written contrast — 39 written to 3 spoken in a blind pilot — could not have been measuring register at all, only provenance; rebuilt as a within-video contrast (a video's transcript segments against that same video's note, split by video) and with code-bearing asked of Haiku over full text instead of grepped from a 900-character head, all five axes pass at 0.91-0.995 against 200-shuffle nulls on 0.50, and signature stability rises to 0.9565; the register axis then had to survive the confound the gate could not see — its poles are nearly disjoint in length and length alone scores 0.977 — which caliper matching kills, dropping length to 0.678 while the probe holds 0.961, with a blind judge reading segments spoken 191 to 9; disclosed against the win: the five spokes are not five directions (mine-world/code-prose abs cosine 0.544) and the code pole is sample-derived |
| 53 | [patch-grid](assets/53-patch-grid/README.md) | #218: SigLIP-2 keeps one pooled vector per image and throws away the 24 x 24 patch grid it came from — do three cheap per-patch maps agree with covering a block and re-encoding? | a registered loss on the wrong material: mean Spearman with the occlusion referee 0.02 / 0.10 / 0.13 against a bar of 0.40, only the gradient-through-the-head lens reads the query (true minus wrong title +0.09, interval [0.03, 0.15]), the owner rated all 60 blind 6 x 6 overlays as misses because nothing at 64-pixel blocks lands on anything, and the referee had under 5% signal on 18 of 40 thumbnails whose titles name nothing you can point at; the section closes with the pivot to the patch-grid night (sections 54-58): ten images with nameable objects, three queries each, pictures at method resolution before any gate |
| 54 | [region-head](assets/54-region-head/README.md) | #218, night 1 of 5: run SigLIP-2's real pooling head once per SAM region with the attention masked to that region (TextRegion) — does it find the named object as well as cropping each region onto grey, at one encoder pass instead of 46? | no, a dead end by its own condition: on the ten images with nameable objects, 19 of 30 hand-written queries are answered by their picture, the two recipes agree on 11 and are right on all of them (goat, trackball, moon, face, rose, sewing machine, mouse, owl), but on the 8 disagreements covering each candidate and re-encoding sides with crop-on-grey 6 to 2, and the head's losses share a shape — the background for frame-filling queries, a single patch or a thin wall strip for small ones — because confined to a region the probe puts a median 47% of its attention on one patch; the full mask reproduces the production vector to cosine > 0.9999, so the head is faithful to the model and not to the object |
| 55 | [long-tokens](assets/55-long-tokens/README.md) | #218, night 2 of 5: the registers phenomenon (Darcet et al.) measured for the first time on SigLIP-2 and an attention-pooling head — where do the long tokens emerge, where do they sit, what does hiding them do to the vector ytk searches with, and does a test-time register work here? | the checkpoint has three built-in register slots: no token above norm 150 through layer 9, then at layer 10 of 27 (37% depth, the paper's number) two tokens jump from ~10 to ~2,100 and a third joins at layer 27, at the same three border patches in 50 of 50 images regardless of content; hiding them from the probe moves every production vector (median cosine 0.979, min 0.948, all 50 below 0.99; three random tokens: 1.000); nine MLP neurons in layer 10 make them (scores 180-212, nothing else above 82), zeroing them costs cosine 0.85, and moving them onto one appended zero token removes every long token at cosine 0.977 with the register taking 8% of the probe's attention; the attention map over the image stays speckle either way |
| 56 | [referee](assets/56-referee/README.md) | #218, night 3 of 5: section 53 judged every cheap map against a 6 x 6 block cover — do two sturdier referees, covering each whole SAM object and RISE's random multi-region covers, rank regions the same way, so the cheap one is enough? | no, and the useful negative is the other way round: whole-object occlusion against RISE scores Spearman -0.28, +0.08, +0.53 on the three RISE images, RISE lights the woman's head, the frame-filling goat and the sewing machine and converges by 1,000 masks (rank 0.97 with 2,000) while the object cover misses the woman for a block of buildings and the machine for the floor, and on the small-object question over all ten the object cover agrees with crop-on-grey on 3 of 10 and picks the larger region in 6 of 7 disagreements — necessity scales with area, so covering whole objects is not a referee for small things; RISE costs 18 min an image here, so section 58 reports against both |
| 57 | [sam3](assets/57-sam3/README.md) | #218, night 4 of 5: does SAM 3, which segments from a text concept in one model, load on the M3 through transformers, how fast is it on MPS, and does it belong in ytk at query time or only in the boxing project on the 3070? | it loads and runs: 0.84B parameters fp16 on MPS at 3.0 GB, 4 to 6 s to load, 2.6 to 4.9 s per prompt, after a licence gate that first refused the download and cleared in twenty minutes once requested; on the woman image it finds the sunglasses at 0.93 (IoU 0.81 with the masked head's pick, 0.02 with crop-on-grey's) and the whole person at 0.96 under "woman", which no SAM-1 region ever covered, and nothing for "the whole woman" or the printed caption; a per-query cost of seconds rules it out behind a cached collection-wide view and in for a single-image one, and the sunglasses result is a recorded caveat on section 54's tiebreaker |
| 58 | [gradient-lens](assets/58-gradient-lens/README.md) | #218, night 5 of 5: can a whole-encoder gradient lens (LeGrad's gradient on every layer's attention, or gradient x activation on every layer's tokens), one forward and one backward with nothing cached, replace the referee for a single-image "show me where" view? | no, a dead end at median Spearman +0.07 and +0.01 against a bar of 0.30: with no CLS row to read, averaging the clamped attention gradient over all 576 query rows gives every patch its area share (peak patch 0.2% of the map on every image), gradient x activation is speckle at every one of the 27 layers with no hill forming, zeroing section 55's three slots changes nothing, and against RISE the best variant scores +0.17, -0.13, +0.17 — on an attention-pooled encoder the score is a function of a probe outside the encoder, so no row inside it is privileged, and every lens that located an object in this record paid for encoder passes |
...
- **48** (spoken-written was real signal killed by a thin pole) ← 52. Chroma
  embeds a YouTube note's enrichment, not its transcript, so both poles of that
  contrast were composed prose — a blind pilot labels the doc corpus 39 written
  to 3 spoken. The 0.95 was provenance wearing a register label, and no amount
  of pole growth could have redeemed it; rebuilt as a within-video contrast the
  axis passes at 0.995 and survives a length control. The loss recorded in 48
  stands and its bar did not move.
