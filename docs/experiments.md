...
| 57 | [sam3](assets/57-sam3/README.md) | #218, night 4 of 5: does SAM 3, which segments from a text concept in one model, load on the M3 through transformers, how fast is it on MPS, and does it belong in ytk at query time or only in the boxing project on the 3070? | stopped at the gate, not the code: transformers 5.5.4 already exports Sam3Model and Sam3Processor with no Triton on that path, but `facebook/sam3` is gated (`manual`) and the logged-in account has no approval — `hf download` returns "Access denied. This repository requires approval" — so no timing, no memory figure and no figure exist; the brief's inference stands untested (the detector is text-conditioned, so it runs per query and cannot replace vectors cached at save time), and the section has no lead PNG by the record's rule for a failure at install |
