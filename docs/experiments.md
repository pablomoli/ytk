...
| 53 | [lsd](assets/53-lsd/README.md) | Garry Tan's LSD mode — forbid pairs inside the cone, raise retrieval randomness, generate ~100 orthogonal combinations, rank by coherence — does it hold here, and what is "orthogonal" in a space where every note leans the same way? | rung 0, gates pending the owner's blind deck: the orthogonal strip |cos|<0.1 holds 1.5% of random pairs raw and 78% centred (the rule only exists after centering); Tan's own mechanism, a Boltzmann tilt at T = background std, moves the median 0.63 std while a p10 tail moves it 1.25 (20 seeds), so ORTHO is the tail and the temperature is disclosed as weak; uniform base draws keep hubs out (max 4 draws per note in any seed); pools frozen at ORTHO 1.25 std below RAND and NEAR 3.32 above; 600 ideas generated and judged: 80% sit closer to their parents' midpoint than to any other note (nearest foreign note 0.33 centred in every pool), the midpoint describes ORTHO children worst (0.40 vs NEAR 0.52), the coherence judge prefers inside-the-cone pairs (post ORTHO-NEAR -0.31, p 0.028) and is lenient on build (58-67 fives per 100), and ORTHO children lean harder into the corpus direction (0.613 vs 0.548), so residual novelty and cone-genericness coexist; G1 (ORTHO judge-top-5 >= 3/5 owner-yes and beats NEAR) and G2 (judge-owner Spearman >= 0.30) registered before generation |
