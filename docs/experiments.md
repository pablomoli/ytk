...
| 49 | [latent-portraits](assets/49-latent-portraits/README.md) | rung 7 replacement, part 2: if a latent's portrait is composited purely from its exemplars' real thumbnails, is it an identity or brown mush? | an identity, decisively: same-latent disjoint-half portraits correlate at 0.75 vs 0.07 for cross-pairs, AUC 0.969 against the registered 0.80, 1,655 latents qualifying — and an unplanned finding: the corpus paints words, ghost typography emerging from agreement (#272 spells "LLM Tokenization" and "SolidGoldMagikarp" out of its own exemplars, #1498 spells "Tool-Use") because channels repeat their thumbnail type; portraits stay evidence-composited, never meaning-rendered, and the pass licenses the passport — the protagonist's complete papers, every stamp earned by a gate across sections 42-49 |
