...
def test_terminal_visible_fires_tmux_and_sketchybar():
    with patch("ytk.memo.shutil.which", return_value="/opt/bin/x"), \
...
def test_no_aerospace_falls_back_to_tmux_env(monkeypatch):
    ...
    with patch("ytk.memo.shutil.which", side_effect=lambda b: None if b == "aerospace" else "/x"), \
...
def test_backend_failure_is_silent(monkeypatch):
    ...
    with patch("ytk.memo.shutil.which", return_value="/x"), \
