...
def test_faithfulness_never_truncates_enrichment(monkeypatch):
    ...
    enr = Enrichment(thesis="t", summary="s", key_concepts=[marker],
                      insights=[], interest_tags=[], key_moments=[])
...
def test_judge_never_truncates_enrichments(monkeypatch):
    ...
    def fake_structured(system, user_prompt, result, **kw):
        ...
        return ev._JudgeResult(winner="A", specificity="x", faithfulness="x",
                               nonredundancy="x", retrievability="x")

    monkeypatch.setattr(ev, "structured", fake_structured)

    marker_a = "DISTINCTIVE_MARKER_A_ABC"
    marker_b = "DISTINCTIVE_MARKER_B_DEF"
    enr_a = Enrichment(thesis=marker_a, summary="s", key_concepts=[], insights=[],
                       interest_tags=[], key_moments=[])
    enr_b = Enrichment(thesis=marker_b, summary="s", key_concepts=[], insights=[],
                       interest_tags=[], key_moments=[])
