...
def _theme(label, weight, centroid, note_ids=None):
    return Theme(id=label, label=label, summary="", weight=weight,
                 note_ids=note_ids or [], exemplar_titles=[], centroid=centroid)
...
def test_match_birth_death():
    old = _snap([_theme("ai", .6, [1, 0, 0]), _theme("css", .4, [0, 1, 0])])
    new = _snap([_theme("ai-coding", .5, [.98, .02, 0]), _theme("lifting", .5, [0, 0, 1])],
...
def test_greedy_one_to_one():
    ...
    old = _snap([_theme("ai", 1.0, [1, 0, 0])])
    new = _snap([_theme("a", .5, [.99, .01, 0]), _theme("b", .5, [.97, .03, 0])])
    d = diff_snapshots(old, new)
...
def test_backfill_from_note_ids(monkeypatch):
    monkeypatch.setattr(synthesis, "_embeddings_by_id",
                        lambda: {"n1": [1.0, 0.0], "n2": [1.0, 0.0]})
...
def test_render_drift_mentions_movement():
    old = _snap([_theme("ai", .2, [1, 0, 0]), _theme("css", .5, [0, 1, 0])])
    new = _snap([_theme("ai", .5, [1, 0, 0]), _theme("css", .2, [0, 1, 0]),
                 _theme("lifting", .3, [0, 0, 1])])
