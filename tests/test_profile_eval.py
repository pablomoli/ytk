...
def _data():
    notes = [
        {
            "id": "support",
            "source_path": "/vault/support.md",
            "source": "instagram",
            "captured_at": "2026-07-16T00:00:00+00:00",
        },
        {
            "id": "held",
            "source_path": "/vault/held.md",
            "source": "instagram",
    ...
    saved = [
        {
            "id": "ig:support",
            "note_path": "/vault/support.md",
            "source": "instagram",
            "embedding": [1.0, 0.0],
        },
        {
            "id": "ig:held",
            "note_path": "/vault/held.md",
            "source": "instagram",
            "embedding": [1.0, 0.0],
        },
    ]
    pending = [
        {
            "id": "https://instagram.test/negative",
            "note_path": "",
            "source": "instagram",
            "embedding": [0.0, 1.0],
        },
...
def test_score_claims_is_multi_positive_ndcg_over_actual_text_embedding():
    ...
    assert score_claims(["matching"], positives, negatives, lambda _: [[1.0, 0.0]]) == 1.0
    assert score_claims(
...
def test_cohort_prefers_uncited_recent_save_and_matches_negative_source():
    ...
    cfg = InterestConfig(profile_eval_positives=1, profile_eval_negatives_per_positive=1)
    cohort = build_cohort(_snapshot(evidence=["support"]), notes, [1, 1], saved, pending, cfg)
...
def test_cohort_joins_legacy_tiktok_visual_without_note_path():
    ...
    assert cohort is not None
    assert [item["id"] for item in cohort.positives] == ["tt:1234567890123456789-thumb"]
